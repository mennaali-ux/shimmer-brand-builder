    ...
    <nav className="sticky top-0 z-50 w-full bg-background/80 backdrop-blur-md border-b border-rose/10">
      <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between gap-6">
        <div className="flex items-center gap-8">
          ...
          <div className="hidden lg:flex gap-6 text-sm font-medium uppercase tracking-widest">
            ...
            <Link to="/shop" search={{ category: "skincare" }} className="hover:text-rose transition-colors">
              {t("nav.skincare")}
            </Link>
            <Link to="/shop" search={{ category: "makeup" }} className="hover:text-rose transition-colors">
              {t("nav.makeup")}
            </Link>
            <Link to="/shop" search={{ category: "haircare" }} className="hover:text-rose transition-colors">
              {t("nav.haircare")}
            </Link>
          </div>
        </div>

        <div className="hidden md:flex items-center bg-pink-light/30 px-4 py-2 rounded-full border border-rose/20 flex-1 max-w-xs">
        ...
        <div className="flex items-center gap-3 text-xs font-bold">
          <button
            ...
            className="flex items-center gap-1 hover:text-rose transition-colors"
          ...
          <Link to="/auth" className="hover:text-rose hidden sm:inline-flex" aria-label="Account">
            <User className="size-4" />
          </Link>
          <Link to="/cart" className="hover:text-rose hidden sm:inline-flex relative" aria-label="Wishlist">
            <Heart className="size-4" />
            {wishCount > 0 && (
              <span className="absolute -top-2 -end-2 bg-rose text-warm-white text-[9px] rounded-full size-4 grid place-items-center">
                {wishCount}
              </span>
            )}
          </Link>
          <Link to="/cart" className="hover:text-rose relative inline-flex" aria-label="Cart">
            ...
            {count > 0 && (
              <span className="absolute -top-2 -end-2 bg-rose-deep text-warm-white text-[9px] rounded-full size-4 grid place-items-center">
                {count}
              </span>
            )}
          </Link>
      ...
      {open && (
        <div className="lg:hidden border-t border-rose/10 bg-background px-6 py-4 flex flex-col gap-3 text-sm uppercase tracking-widest">
          ...
          <Link to="/auth" onClick={() => setOpen(false)}>{t("nav.account")}</Link>
        </div>
      )}
    </nav>
  );
